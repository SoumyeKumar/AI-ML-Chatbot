...
def create_or_update_profile(user_id, name, email, phone, language, specialization):
    profiles = pd.read_csv(USER_PROFILE_FILE)
    existing_profile = profiles[profiles["UserID"] == user_id]

    if not existing_profile.empty:
        profiles.loc[profiles["UserID"] == user_id, ["Name", "Email", "Phone", "PreferredLanguage", "Specialization"]] = [
            name, email, phone, language, specialization
        ]
        message = "Profile updated successfully."
    else:
        new_profile = {"UserID": user_id, "Name": name, "Email": email, "Phone": phone, 
                       "PreferredLanguage": language, "Specialization": specialization}
        profiles = profiles.append(new_profile, ignore_index=True)
        message = "Profile created successfully."

    profiles.to_csv(USER_PROFILE_FILE, index=False)
    return message

def get_profile(user_id):
    profiles = pd.read_csv(USER_PROFILE_FILE)
    profile = profiles[profiles["UserID"] == user_id]
    return profile.iloc[0].to_dict() if not profile.empty else "Profile not found."