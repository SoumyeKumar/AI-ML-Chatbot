...
from utils import (
    ...
    extract_placeholders,  # Extract placeholders for legal forms
    fill_docx_template,  # Fill legal forms with user input
)
...
async def call_model(state: dict):  # Treat state as a dictionary
    ...
    # Check if the query is to fill a legal form
    elif "fill form" in query.lower() or "complete form" in query.lower():
        try:
            form_path = "path_to_form.docx"  # Replace with dynamic input
            placeholders = extract_placeholders(read_docx(form_path))
            if not placeholders:
                return {"messages": [AIMessage(content="No placeholders found in the form to fill.")]}

            response_content = (
                "To complete this form, please provide the following information:\n" +
                "\n".join([f"- {field}" for field in placeholders])
            )
            return {"messages": [AIMessage(content=response_content)]}

        except Exception as e:
            print(f"Error during form filling: {e}")
            return {"messages": [AIMessage(content="An error occurred while processing the form. Please try again later.")]}
